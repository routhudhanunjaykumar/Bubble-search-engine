...
if ($max_page_number >= 2) { // if more than 2 pages 
  if ($page_number > 0 ) { //Previous
    ?>
   <button type="submit" name="page" value='<?php echo ($page_number -  $results_per_page);?>'>Previous</button>
   <?php
  }
  ...
  for($index = $k; $index < $max_page_number && $index<$k+10; $index++)
    {
		 ...
		 ?>
   <button type="submit" name="page" value='<?php echo ($index * $results_per_page);?>'><?php echo ($index + 1);?></button>
   <?php
    }
  if (($page_number + $results_per_page) < $number_of_result ) { //Next
    ?>
   <button type="submit" name="page" value='<?php echo ($page_number +  $results_per_page);?>'>Next</button>
   <?php
  }
} 
echo "</ul>";
