	...
	<main>
		...
		<div class="forms">
			<form class="login-form login-button-active" action="#" method="post" data-action="animated">
				...
				<input type="submit" name="feedback_btn" value="Login">
			</form>
			<form class="signup-form" action="#" method="post" data-action="animated">
				...
				<input type="submit" name="complaint_btn" value="Continue">
			</form>
		</div>
	</main>
...
<?php
	 ...
	 $db = mysqli_connect("localhost","root","","search");
        ...
        $sql = "INSERT INTO feedack(email, feedback) VALUES('$email', '$feedback')";
        if(mysqli_query($db,$sql))
		echo "<script>alert('Thank you for your valule feedback')</script>";
      else { 
...
if(isset($_POST['complaint_btn'])){
	 ...
	 $db = mysqli_connect("localhost","root","","search");
        ...
        if(mysqli_query($db,$sql))
		echo "<script>alert('Thank you we will solve with in gap')</script>";
      else { 
				echo "<script>alert('error please try again after some time')</script>";
	  }
}
